<template>
  <form
    class="w-full bg-white rounded-lg p-2 pb-5 commentaire"
    @submit.prevent="addComment"
  >
    <div class="flex items-center justify-between">
      <h3 class="font-bold text-xl">Commentaire</h3>
      <div class="flex gap-2">
        <q-btn color="blue" icon="autorenew" @click="allCommentInThisWeek" />
    ...
    <div class="oveflow-auto">
      <div class="flex flex-col gap-5">
        <div
          class="max-h-[450px] overflow-auto"
          v-if="commentStore.allComments.length"
        >
          <div
            class="flex w-full justify-between rounded-md border mb-1 flex relative"
            v-for="(item, i) in commentStore.allComments"
            :key="i"
          >
            <div
              class="absolute left-0 top-0 w-[5px] bg-red-500 rounded-l-lg h-full"
              v-if="userDash.id == item.id"
            ></div>
            <div
              class="flex items-center gap-2 w-full p-1 px-2 relative"
              v-if="!commentUpdate"
            >
            ...
            <div class="px-2 w-full">
              <div class="flex items-center w-full">
                ...
                <div class="text-black w-full p-2" v-else>
                  {{ item.comment }}
                </div>
              </div>
            </div>
        ...
        <div class="h-[450px] flex items-center justify-center" v-else>
          Pas de commentaire
        </div>
      </div>
    </div>
...
.commentaire {
  display: grid;
  height: 100%;
  grid-template-rows: 50px calc(100% - 160px) 110px;
}
</style>
